...
/// <summary>
///     A network session that represents a connection to a remote endpoint.
/// </summary>
public abstract class BaseSession : IAsyncDisposable
{
	private static readonly ILogger Logger = Log.ForContext<BaseSession>();
	...
	/// <summary>
	///     Gets the unique identifier of the underlying session.
	/// </summary>
	public string SessionId =>
		_sessionId ??= Uuid.New();

	/// <summary>
	///     Gets the remote endpoint of the underlying session.
	/// </summary>
	...
	internal async Task ReceiveAsync()
	{
			...
			while (!_cts.IsCancellationRequested)
			{
				...
				try
				{
					if (_messageParser.TryDecodeMessage(buffer, out var message))
						await _messageDispatcher.DispatchAsync(this, message);
				}
				finally
				{
		...
		{
			Logger.Error($"An error occured. {e.GetType().Name}: {e.Message} {e.StackTrace}");
		}
	}
